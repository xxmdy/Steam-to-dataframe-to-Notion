...
run_upload_loop <- function(df, database_id, token,
  ...
  for (i in seq_len(nrow(df))) {
    ...
    if (is.null(row$游戏名称) || is.na(row$游戏名称) || row$游戏名称 == "" || all(is.na(row))) {
      log$warning(glue::glue("⏩ [{i}/{nrow(df)}] 跳过无效记录（无游戏名）"))
      skip <- TRUE
    }
    ...
    if (!skip) {
      ...
      # 构建 Notion 所需属性
      props <- notion$build_notion_props(row, status = game_status)
      ...
      if (result$success) {
        ...
        account <- safe$safe_text(row$所属账户)
        log$success(glue::glue("✅ [{i}/{nrow(df)}] 游戏「{display_name}」（{game_id_str}）[{account}]：{result$action}"))
        log$log_to_file(glue::glue("✅ [{i}/{nrow(df)}] 游戏「{game_name}」（ID: {game_id_str}）[{account}]上传成功：{result$action}"), type = "SUCCESS")
      } else {
        fail_count <- fail_count + 1
        log$error(glue::glue("❌ [{i}/{nrow(df)}] 游戏「{display_name}」（{game_id_str}）[{account}]：{result$action}"))
        log$log_to_file(glue::glue("❌ [{i}/{nrow(df)}] 游戏「{game_name}」（{game_id_str}）[{account}]上传失败：{result$action}"), type = "ERROR")
      }
    }
